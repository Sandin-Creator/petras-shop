...
export const UPLOAD_DIR = ensureWritableDir(ENV_OR_PUBLIC, LOCAL_FALLBACK);
...
const fileFilter = (_req: any, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  if (!IMAGE_MIME.test(file.mimetype || "")) {
    return cb(new MulterError("LIMIT_UNEXPECTED_FILE", "Only image uploads are allowed"));
  }
  cb(null, true);
};
...
router.post(
  ...
  (req, res) => {
    // Multer adds req.files as Dict<string, File[]>
    ...
    // Public URL is served by server.js
